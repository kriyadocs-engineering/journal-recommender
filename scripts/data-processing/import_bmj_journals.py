#!/usr/bin/env python3
"""
Import BMJ journals with scope into the PostgreSQL database.
"""
...
INPUT_JSON = DATA_DIR / "bmj_journals_with_scope.json"
...
def main():
    print("=" * 60)
    print("BMJ Journals Database Import")
    print("=" * 60)
    ...
    # Verify import
    cur.execute("SELECT COUNT(*) FROM journals WHERE publisher = 'BMJ Publishing Group'")
    bmj_count = cur.fetchone()[0]

    cur.execute("""
        SELECT COUNT(*) FROM journals j
        JOIN journal_scopes js ON j.id = js.journal_id
        WHERE j.publisher = 'BMJ Publishing Group'
    """)
    bmj_with_scope = cur.fetchone()[0]

    print(f"\n5. Database verification:")
    print(f"   BMJ journals in DB: {bmj_count}")
    print(f"   BMJ journals with scope: {bmj_with_scope}")
