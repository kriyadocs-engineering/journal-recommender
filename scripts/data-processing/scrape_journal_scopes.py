#!/usr/bin/env python3
"""
Scrape journal scope/aims from Scimago journal pages.
...
INPUT_CSV = DATA_DIR / "world_scientific_journals.csv"
OUTPUT_JSON = DATA_DIR / "world_scientific_journals_with_scope.json"
...
def get_journal_scope(source_id: str) -> Optional[str]:
    ...
    try:
        ...
        # Look for the scope/description section
        # Scimago typically has this in a specific div
        scope_div = soup.find('div', class_='journaldescription')
        if scope_div:
            return scope_div.get_text(strip=True)

        # Alternative: look for description in meta tags
        meta_desc = soup.find('meta', attrs={'name': 'description'})
        if meta_desc and meta_desc.get('content'):
            return meta_desc['content']

        # Try finding any element with scope-related text
        for elem in soup.find_all(['p', 'div']):
            text = elem.get_text(strip=True)
            if len(text) > 100 and any(word in text.lower() for word in ['scope', 'aims', 'publishes', 'journal covers']):
                return text
...
def scrape_all_scopes(journals: list[dict], delay: float = 1.0) -> list[dict]:
    ...
    for i, journal in enumerate(journals):
        ...
        scope = get_journal_scope(journal['source_id'])
        journal['scope'] = scope
...
def main():
    print("=" * 60)
    print("World Scientific Journal Scope Scraper")
    print("=" * 60)

    # Parse CSV
    print("\n1. Parsing CSV...")
    journals = parse_csv()
    print(f"   Found {len(journals)} World Scientific journals")
